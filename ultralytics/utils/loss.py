...
class v8DetectionLoss:
    ...
    def clpf_loss(self):
        """Collect cross-modal latent prediction losses from CLPF modules."""
        losses = [
            m.cross_modal_loss
            for m in self.model.modules()
            if hasattr(m, "cross_modal_loss") and m.cross_modal_loss is not None
        ]
    ...
    def mf_sigreg_loss(self):
        """Collect train-time MFSIGReg losses from identity regularization modules."""
        losses = [
            m.reg_loss
            for m in self.model.modules()
            if hasattr(m, "reg_loss") and m.reg_loss is not None
        ]
        if not losses:
            return torch.zeros((), device=self.device)
        return torch.stack(losses).mean()
    ...
    def preprocess(self, targets, batch_size, scale_tensor):
        ...
        if nl == 0:
            ...
            # NOTE: targets is float after cat([batch_idx, cls, bboxes]); cast index column back to int for robust matching.
            i = targets[:, 0].long()  # image index
            _, counts = i.unique(return_counts=True)
            counts = counts.to(dtype=torch.int32)
            out = torch.zeros(batch_size, counts.max(), ne - 1, device=self.device)
            for j in range(batch_size):
                matched_idx = (i == j).nonzero(as_tuple=False).squeeze(1)
                n = int(matched_idx.numel())
                if n:
                    out[j, :n] = targets[matched_idx, 1:]
    ...
    def __call__(self, preds, batch):
        ...
        clpf_loss = self.clpf_loss() * getattr(self.hyp, "clpf_loss_weight", 0.0)
        mf_sigreg_loss = self.mf_sigreg_loss() * getattr(self.hyp, "mf_sigreg_loss_weight", 1.0)

        total_loss = (loss.sum() + clpf_loss + mf_sigreg_loss) * batch_size
        all_losses = torch.cat([loss.detach(), clpf_loss.detach().unsqueeze(0), mf_sigreg_loss.detach().unsqueeze(0)])
